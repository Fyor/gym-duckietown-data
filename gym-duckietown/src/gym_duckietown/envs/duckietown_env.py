...
class DuckietownForward(Simulator):
    """
    Environment for the Duckietown lane following task with
    and without obstacles (LF and LFV tasks)
    """

    def __init__(self, chase_factor=0.1, chase_stuck_range=0.075, **kwargs):
        self.q = deque()

        self.chase_factor = chase_factor  # lower is faster chase
        self.chase_stuck_range = chase_stuck_range  # higher is larger stuck detection range

        super().__init__(**kwargs)
        # 0.1 m every step
        # 30 steps then reset
        # Punishment rampup to -10

    def reset(self, segment=False):
        x = super().reset(segment)
        self.q.clear()
        return x
    ...
    def step(self, action):
        ...
        self.q.append(self.cur_pos)

        if len(self.q) > 300:
            chase_pos = list(self.q)[:-50]
            self.q.popleft()

            dist = np.linalg.norm(chase_pos - self.cur_pos, axis=1)
            dist = dist.mean()


            # if stuck in same place
            # if np.sum(dist < self.chase_stuck_range) > 5:
            #     reward -= 25
            #     info['Simulator']['msg'] += "Stuck in same place"
            #     done = True
            #     return obs, reward, done, info


            max_punishment = 6
            dist_punishment = max_punishment - (dist * 25)
            dist_punishment = dist_punishment.clip(0, max_punishment)
            # print("Dist punish", -dist_punishment)
            info["dist_punish"] = -dist_punishment
            reward -= dist_punishment
            reward = min(0, reward)
            # reward += dist * 5
        return obs, reward, done, info
