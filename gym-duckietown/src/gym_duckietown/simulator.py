...
class Simulator(gym.Env):
    ...
    def reset(self, segment=False):
        ...
        if self.start_pose is not None:
            ...
            for _ in range(MAX_SPAWN_ATTEMPTS):
                ...
                ok = -M < lp.angle_deg < +M
                ok = ok and abs(lp.dist) < MAX_LP_DIST # DO NOT SPAWN OUTSIDE OF LANE
                if not ok:
                    continue
    ...
    def compute_reward(self, pos, angle, speed, return_lp=False):
    ...
    def _compute_done_reward(self) -> DoneRewardInfo:
        ...
        # If the maximum time step count is reached
            ...
            # print("ROBOT SPEED", self.robot_speed, "SPEED", self.speed)
            reward, lp = self.compute_reward(self.cur_pos, self.cur_angle, self.speed, return_lp=True) # FIX BUG WHERE PASSING self.robot_speed
            msg = ""
            done_code = "in-progress"

            if abs(lp.dist) > MAX_LP_DIST:
                done = True
                msg = "Stopping the simulator because the bot crossed the lane"
                reward = REWARD_INVALID_POSE
                done_code = "lane-crossing"
